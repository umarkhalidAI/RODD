...
def main():
    ...
    data_out = ['LSUN_crop', 'LSUN_resize', 'Imagenet_resize', 'Imagenet_crop']
    ...
    for level in range(1, 6):
        ...
        for cor in CORRUPTIONS:
            for dataName in data_out:
                out_data = dataName + cor
                exp_dir = "../features/" + args.path + str(level)
                f, p = process_features(exp_dir, out_data, train, test_in, labels, cor, level)
                fpr.append(f)
                aur.append(p)
        print(level, ' : ', np.mean(fpr))
        print(level, ' : ', np.mean(aur))


def process_features(exp_dir, out_dataset, train, test_in, labels, cor, level):
    test = np.squeeze(np.load(exp_dir + '/features_out_' + out_dataset + '.npy'))
    print(level, '_', out_dataset)
