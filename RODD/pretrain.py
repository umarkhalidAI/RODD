...
from utils import * # logger, pgd_attack,eval_adv_test,setup_seed,AverageMeter,pair_cosine_similarity,nt_xent
import torchvision.transforms as transforms
from models import wideresnet_UoS,projector
import numpy as np

from cifar10 import CustomCIFAR10, CustomCIFAR100
from lars import LARS
import os

parser = argparse.ArgumentParser(description='Contrastive Pre-Training')
parser.add_argument('--experiment',default='pretrain', type=str, help='location for saving trained models')
...
def main():
    ...
    model=wideresnet_UoS.WideResNet(40,num_classes,2, 0.3)
...
def train(train_loader, model, optimizer, scheduler, epoch, log, num_classes):
    ...
    for i, (inputs) in enumerate(train_loader):
        ...
        logi, features_adv=model.train()(inputs_adv)
        #features_adv = model.train()(inputs_adv)
        logit, features = model.train()(inputs)

        #print(features.shape,features_adv.shape)
...
def validate(train_loader, val_loader, model, log, num_classes=10):
    ...
    for loader in [train_loader, val_loader]:
        ...
        for i, (inputs, targets) in enumerate(loader):
            ...
            # compute output
            with torch.no_grad():
                outputs,fea = model.eval()(inputs, 'normal')
...
def PGD_contrastive(model, inputs, eps=8. / 255., alpha=2. / 255., iters=10, singleImg=False, feature_gene=None, sameBN=False):
    ...
    for i in range(iters):
        logits,features = model.eval()(inputs + delta)
        model.zero_grad()
